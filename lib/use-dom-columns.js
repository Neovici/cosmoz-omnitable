...
const
	...
	useDOMColumns = (host, { enabledColumns, config }) => {
		const
		...
		return useMemo(
			() => [...Object.entries(config).map(([name, column]) => ({
				...defaultColumn,
				...column,
				name,
				valuePath: column.valuePath ?? name,
				title: column.title ?? name,
				sortOn: column.sortOn ?? column.valuePath ?? name,
				groupOn: column.groupOn ?? column.valuePath ?? name
			})), ...columns],
			[config, columns]
		);
	};
