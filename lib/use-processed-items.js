...
export const useProcessedItems = ({ data, columns, groupOnColumn, groupOnDescending, sortOnColumn, descending, hashParam }) => {
	const
		...
		indexedData = useMemo(() => {
			...
			return data.map((item, index) => ({...item, [indexSymbol]: index}))
		}, [data]),
		...
		filteredItems = useMemo(() => {
			...
			return indexedData.filter(item => Object.values(filterFunctions).every(filterFn => filterFn(item)));
		}, [indexedData, filterFunctions]),
