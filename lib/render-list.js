...
const exportParts = ({ columns, collapsed }) => {
	const showingColumns = columns.filter(
		(column) => !collapsed.some((hidden) => hidden.name === column.name),
	);

	return showingColumns.map((col) => `cell-${col.name}`);
};
			...
			() =>
				html`<div class="tableContent-empty overlay">
					<cosmoz-omnitable-skeleton
						exportparts=${exportParts(settingsConfig)}
