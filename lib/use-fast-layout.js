...
export const useFastLayout = host => {
	const
		...
		columnLayout = useMemo(() => {
			...
			const columnConfigs = columns
				.map(column => ({
					basis: column.resizeWidth ?? parseInt(column.width, 10),
	...
	useEffect(() => {
		const refreshConfig = () => {
			host.columns = host.columns != null && Array.from(host.columns);
		};
		host.addEventListener('cosmoz-column-prop-changed', refreshConfig);
		return () => host.removeEventListener('cosmoz-column-prop-changed', refreshConfig);
	}, []);
