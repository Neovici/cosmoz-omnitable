...
const intialState = {
	...
	handleUpdate = (config, dimensions, hiddenColumns, containerWidth) => {
		const lastHiddenColumn = hiddenColumns.slice(hiddenColumns.length - 1)
		// unhide the last hidden column, if the breakpoint has been passed
		let newHiddenColumns = lastHiddenColumn.length !== 0 && lastHiddenColumn[0].breakpoint < containerWidth
		...
		if (tooSmall && !isLastColumn) {
			...
			newHiddenColumns.push({ id: droppedColumn, breakpoint: containerWidth });
		}
