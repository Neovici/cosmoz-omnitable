...
const
	_toCss = (layout, config) =>
		layout
			.map((width, index) =>
				width == null || width === 0
					? `cosmoz-omnitable-resize-nub[name="${ config[index].name }"], .cell[name="${ config[index].name }"]{display:none}`
					: `.cell[name="${ config[index].name }"]{width: ${ width }px;padding: 0 min(3px, ${ width / 2 }px)}`
			)
			.join('\n');
