...
suite('Basic omnitable functionality', () => {
				...
				>
					...
					<basic-column
						...
						.renderCell=${(column, { item }) =>
							html`<span class="overriden-column-cell"
								>Overriden ${item.name}</span
							>`}
						.renderHeader=${() =>
							html`<span class="overriden-column-header"
								>Overriden Header</span
							>`}
					></basic-column>
	...
	test('should display headers as configured', () => {
		const headers = Array.from(
			omnitable.shadowRoot.querySelectorAll(
				'.basic-column-header, .overriden-column-header',
			),
		);
		expect(headers).to.not.be.empty;
		expect(headers.length).to.equal(2);
		expect(headers.map((i) => i.innerText)).to.deep.equal([
			'Header',
			'Overriden Header',
		]);
	});
	...
	test('should display cells as configured', () => {
		const cells = Array.from(
			omnitable.shadowRoot.querySelectorAll(
				'.basic-column-cell, .overriden-column-cell',
			),
		);
		expect(cells).to.not.be.empty;
		expect(cells.length).to.equal(6);
		expect(cells.map((c) => c.innerText)).to.deep.equal([
			'item 1',
			'Overriden item 1',
			'item 2',
			'Overriden item 2',
	...
	test('should use editable template when the column is editable', async () => {
		...
		const cells = Array.from(
			omnitable.shadowRoot.querySelectorAll('.basic-column-cell'),
		);
		expect(cells.map((c) => c.innerText)).to.deep.equal([
			'Edit: item 1',
			'Edit: item 1',
