...
suite('render', () => {
	test('basic render', async () => {
		const omnitable = await setupOmnitableFixture(rangeFixture, data),
			cells = Array.from(omnitable.shadowRoot.querySelectorAll('.itemRow-cell'));
...
suite('date', () => {
	...
	setup(async () => {
		...
		columnHeaderInput = omnitable.shadowRoot.querySelector('cosmoz-omnitable-date-range-input');
	});
	...
	test('changing filter updates _filterInput', async () => {
		...
		assert.equal(fMin, toLocalISOString(min).slice(0, 10), 'Expect _filterInput.min to be local ISO String');
		assert.equal(fMax, toLocalISOString(max).slice(0, 10), 'Expect _filterInput.max to be local ISO String');

		assert.equal(toDate(fMin).getTime(), min.getTime(), 'Expect min input and filter to be equal as dates');
		assert.equal(toDate(fMax).getTime(), max.getTime(), 'Expect max input and filter to be equal as dates');
	});

	test('changing _filterInput updates filter', async () => {
		const columnHeaderInput = omnitable.shadowRoot.querySelector('cosmoz-omnitable-date-range-input');
...
suite('date - pure functions', () => {
	test('toDate converts an value to valid date', () => {
		assert.equal(toDate('2023-03-21T00:00:00Z').getTime(), new Date('2023-03-21T00:00:00Z').getTime());
		assert.equal(toDate(new Date('2015-03-18T00:00:00Z')).getTime(), new Date('2015-03-18T00:00:00Z').getTime());
		assert.equal(toDate(new Date('2017-09-23T01:02:03Z')).getTime(), new Date('2017-09-23T01:02:03Z').getTime());
		assert.equal(toDate(new Date(86400000)).getTime(), new Date(Date.UTC(1970, 0, 2)).getTime());
	});

	test('toDate limits parameters', () => {
		assert.equal(
			toDate('2023-03-21T00:00:00Z', '2023-03-21T00:00:00Z', Math.min).getTime(),
			new Date('2023-03-21T00:00:00Z').getTime()
		); // min
		assert.equal(
			toDate(new Date('2015-03-18T00:00:00Z'), '2053-04-21T00:00:00Z', Math.min).getTime(),
			new Date('2015-03-18T00:00:00Z').getTime()
		); // min
		assert.equal(
			toDate('2023-03-21T00:00:00Z', '2023-04-21T00:00:00Z', Math.max).getTime(),
			new Date('2023-04-21T00:00:00Z').getTime()
		); //max
		assert.equal(
			toDate(new Date('2015-03-18T00:00:00Z'), '2017-03-18T00:00:00Z').getTime(),
			new Date('2015-03-18T00:00:00Z').getTime()
	...
	test('_toLocalISOString returns formated date', () => {
		[
			...
			assert.equal(
				...
				toDate(date.getTime() - date.getTimezoneOffset() * 6e4).toISOString().slice(0, 23)
			);
		});
	});
});
