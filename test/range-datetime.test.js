...
suite('datetime', () => {
	...
	setup(async () => {
		...
		columnHeaderInput = omnitable.shadowRoot.querySelector('cosmoz-omnitable-datetime-range-input');
	});
...
suite('datetime - pure functions', () => {
	...
	test('_toInputString converts datetime to local string', () => {
		[
			...
			assert.equal(toInputString(value), toLocalISOString(toDate(value)).slice(0, 19))
		);
	});
	...
	test('_toHashString converts datetime to UTC string replacing : with .', () => {
		[
			...
			assert.equal(toHashString(date), toDate(date).toISOString().substr(0, 19)
				.replace(/:/gu, '.'))
		);
	});
});
