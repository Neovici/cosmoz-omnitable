...
/**
 * Converts a value to an amount object optionally limiting it.
 */
export const toAmount = (
	rates: Rates = {},
	value: AmountValue | number | null | '',
	limit?: Limit | null,
	limitFunc?: LimitFunc,
): AmountValue | null => {
	console.log('toAmount', { rates, value, limit, limitFunc });
	...
	if (lAmount == null) return amount;

	const valAmount = (amount.amount as number) * (rates[amount.currency] || 1);
	const limAmount = (lAmount.amount as number) * (rates[lAmount.currency] || 1);
...
export const getComparableValue = (
	{ valuePath, rates }: { valuePath?: string; rates?: Rates },
	item: Item,
): number | undefined => {
	if (item == null) {
		return;
