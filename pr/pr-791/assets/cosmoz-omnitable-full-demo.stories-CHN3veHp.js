import{x as c,B as ke,E as gt,b as Mt}from"./iframe-Be3xGFcS.js";import{t as K,c as E,g as C,P as A,u as M,a as ht,m as ft,s as Nt,D as jt,b as Pt,e as Bt,i as be,h as Oe,d as Ht,f as Gt,j as Wt,k as $,n as qt,l as Xt,o as bt,p as Ve,q as I,r as T,v as me,w as Ut,x as xt,y as Kt,z as Jt,A as _,B as Qt,C as xe,E as pe,F as V,G as Yt,H as ye,I as Zt,J as en,K as tn,T as nn,L as yt,M as De,N as on,O as vt,Q as rn,R as Ae,S as Ue,U as ln,V as Ee,W as an,X as sn,Y as _e,Z as Te,_ as un,$ as Ie,a0 as P,a1 as cn,a2 as $e,a3 as dn}from"./cosmoz-collapse-DDCAkEqp.js";const mn=K`
	:host {
		display: flex;
	}
	:host(:not([light])) {
...
`,jn=()=>c`
	<sort-and-group-consumer
		class="sgs"
		.render=${({columns:n,sortOn:e,setSortOn:t,descending:o,setDescending:r}={})=>kt({columns:n?.filter?.(i=>i.sortOn&&!i.noSort),on:e,setOn:t,descending:o,setDescending:r})}
	>
	</sort-and-group-consumer>
...
`,Hn=(n,e)=>c`<div id="input" part="input" role="button" class="control" slot=${e}>
	...
	`};class qn extends Le(Z(A)){static get properties(){return{currency:{type:String},autodetect:{type:Boolean,value:!1},rates:{type:Object},autoupdate:{type:String,value:!1},_filterText:{type:String,computed:"_computeFilterText(filter.*, _formatters)"},headerFocused:{type:Boolean,value:!1},column:{type:Object,value:null}}}static get observers(){return["_valuesChanged(autodetect, currency, values)"]}render(){const e=t=>{this.headerFocused=t.type==="focus",this._onDropdownOpenedChanged(t)};return c`
			<style>
...
`;customElements.define("cosmoz-omnitable-item-expand-line",E(Go,{styleSheets:[De(Ho)]}));const Wo=({columns:n,item:e,selected:t,expanded:o,groupOnColumn:r})=>on(n,i=>c`<cosmoz-omnitable-item-expand-line
			...
			>`);customElements.define("cosmoz-omnitable-item-expand",E(Wo,{useShadowDOM:!1}));const qo=(n,e,t)=>n.editable?n.renderEditCell(n,e,t(n,e.item)):n.renderCell(n,e),Xo=({columns:n,groupOnColumn:e,item:t,index:o,selected:r,expanded:i,onItemChange:l})=>yt(n,a=>a.name,a=>c`<div
...
`);l.unshift(i),vt(new File(l,t,{type:"text/csv;charset=utf-8"}))},Jo=(n,e)=>{const t=n.map(r=>r.title),o=e.map(r=>n.map(i=>{const l=i.toXlsxValue(i,r);return l??""}));return o.unshift(t),o},Qo=(n,e,t,o)=>{const r=Jo(n,e),i=new rn(t).addSheetFromData(r,o).generate();vt(new File([i],t,{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}))},Yo=({columns:n,selectedItems:e,csvFilename:t,xlsxFilename:o,xlsxSheetname:r,topPlacement:i})=>c`<cosmoz-bottom-bar
		id="bottomBar"
		?active=${!Ae(e.length)}
		part="bottomBar"
...
`;customElements.define("cosmoz-omnitable-settings",E(cr,{styleSheets:[De(ir)]}));const ge=["sortOn","descending","groupOn","groupOnDescending"],ue=n=>e=>typeof e=="object"&&e!==null&&"name"in e&&e.name===n,dr=(n=[],e=[],t=[])=>{const o=e.filter(l=>n.some(ue(l.name))),r=n.filter(l=>l.name!=null&&!e.some(ue(l.name))&&!t.some(ue(l.name))),i=t.filter(l=>!e.some(ue(l.name)));return[...o,...i.flatMap(l=>{const a=n.find(s=>s.name===l.name);return a?{...l,title:a.title??l.title??"",minWidth:parseInt(a.minWidth??"0",10)}:[]}),...r.map(l=>{const{name:a,title:s,priority:u,minWidth:m,width:d,flex:g}=l;return{name:a??"",title:s??"",priority:u??0,minWidth:parseInt(m??"0",10),width:parseInt(d??"0",10),flex:parseInt(g??"0",10)}})]},mr=(n,e)=>({...e,..._e(Array.from(ge))(n),columns:n.columns?.map(_e(["name","priority","width","flex","disabled"]))}),pr=({columns:n,settings:e,savedSettings:t,initial:o})=>({...Object.fromEntries(ge.flatMap(i=>o?.[i]!=null?[[i,o[i]]]:[])),...t?_e(Array.from(ge))(t):{},...e,columns:dr(n,e?.columns,t?.columns)}),gr=({prefix:n="omnitable-"}={})=>({write:async(t,o)=>{const r=n+t;try{o?localStorage.setItem(r,JSON.stringify(o)):localStorage.removeItem(r)}catch(i){console.error(i)}},read:async t=>{if(t)try{const o=localStorage.getItem(n+t);return o==null?void 0:JSON.parse(o)}catch(o){console.error(o)}}}),hr=bt(gr),fr=()=>{const n=Ve(hr);return I(()=>n(),[n])},br=(n,e,t,o)=>{const[r,i]=M(),{read:l,write:a}=fr();return T(async()=>{n&&i(await l(n))},[n,l]),{settingsId:n,savedSettings:r,onSave:$(async()=>{if(!n)return;const s=mr(e,r);await a(n,s),t(),i(s)},[e,r]),onReset:$(async s=>{t(),s.shiftKey&&(await a(n),i()),o?.()},[o]),hasChanges:e!=null}},xr=({settingsId:n,host:e})=>{const t=I(()=>Object.fromEntries(ge.map(b=>[b,e[b]])),[]),o=Te(),r=$(()=>{o.current?.(t)},[t]),[i,l]=M(),[a,s]=M({columns:!0,sort:!0}),{savedSettings:u,...m}=br(n,i,l,r),{enabledColumns:d}=e,g=On(e,{enabledColumns:d}),p=I(()=>pr({columns:g,settings:i,savedSettings:u,initial:t}),[g,i,u]),w=I(()=>p.columns.map(b=>g.find(x=>x.name===b.name)).filter(Boolean),[g,...p.columns.map(b=>b.name)]);return{...m,opened:a,setOpened:s,settings:p,columns:w,setSettings:l,resetRef:o}},dt=n=>Number.isFinite(n)?n:0,yr=(n,e)=>{const t=[];let[o,r]=n.reduce(([m,d],{width:g,flex:p})=>[m+g,d+p],[0,0]),i=e-o,l=dt(i/r),a=0,s=0,u=0;for(let m=0;m<n.length;m++){const{width:d,minWidth:g,flex:p}=n[m],w=i>=0?l*p:d*i/o;if(g>d+w){a+=d,s+=g,u+=p,t[m]=g;continue}if(p===0){a+=d,s+=d,t[m]=d;continue}}o-=a,i=e-s-o,r-=u,l=dt(i/r);for(let m=0;m<n.length;m++){if(t[m]!=null)continue;const{width:d,flex:g}=n[m],p=i>=0?l*g:d*i/o;t[m]=d+p}return t},he=Symbol("index"),vr=(n,e)=>{if(typeof n.findLastIndex=="function")return n.findLastIndex(e);for(let t=n.length-1;t>=0;t--)if(e(n[t],t,n))return t;return-1},$r=(n,e)=>{const t=vr(n,l=>l!=null&&l>0),o=(l,a)=>`.cell[name="${l}"], cosmoz-omnitable-skeleton::part(cell-${l}){width: ${Math.floor(a)}px;padding: 0 min(3px, ${a/2}px)}`,r=l=>`cosmoz-omnitable-resize-nub[name="${l}"]{display:none}`,i=l=>`cosmoz-omnitable-resize-nub[name="${l}"], .cell[name="${l}"]{display:none}`;return e.map((l,a)=>{const s=n[a];if(s==null||s===0)return i(l.name);const u=o(l.name,s);return a===t?`${u}
${r(l.name)}`:u}).join(`
`)},Tt=(n,e,t)=>{const o=n.filter(a=>!a.hidden),r=o.reduce((a,{width:s})=>a+s,0);if(o.length>1&&r>e)return Tt(o.slice(1),e,t);const i=o.reduce(([a,s],u,m)=>[Math.max(a,u.index),u.index>a?m:s],[-1,-1])[1];return i!==-1&&(o[i].flex=1),yr(o,e).reduce((a,s,u)=>(a[o[u].index]=s,a),new Array(t).fill(void 0))},wr=(n,e)=>n.length===0?".cell {display: none;}":$r(n,e),zr=({host:n,canvasWidth:e,layout:t,setSettings:o})=>{const r=Te();r.current=i=>o(l=>{const a=l.columns,{detail:{newWidth:s,column:u}}=i,m=a.findIndex(p=>p.name===u.name),d=[],g=a.reduce((p,w)=>Math.max(p,w.priority),-1/0);for(let p=0;p<t.length;p++)if(d[p]={...a[p]},p<m&&t[p]&&(d[p].width=t[p],d[p].flex=0,d[p].priority=g),p===m){const w=t.reduce((b,x,F)=>F<m&&x?b-x:b,e);d[p].width=Math.min(w,Math.max(s,a[p].minWidth)),d[p].flex=0,d[p].priority=g}return{...l,columns:d}}),T(()=>{const i=l=>r.current?.(l);return n.addEventListener("column-resize",i),()=>n.removeEventListener("column-resize",i)},[])},Cr=(n,e)=>T(()=>{const t=([r])=>{r.contentRect?.width!==0&&requestAnimationFrame(()=>e(r.contentRect?.width-20-44-24))},o=new ResizeObserver(t);return o.observe(n),()=>o.unobserve(n)},[]),Sr=n=>{const[e,t]=M(()=>n.getBoundingClientRect().width);return Cr(n,t),e},kr=(n,e)=>{const t=I(()=>{let o=!1,r;const i=()=>{if(!o)return;r=requestAnimationFrame(i),n()&&(o=!1)};return{start:()=>{o=!0,cancelAnimationFrame(r),r=requestAnimationFrame(i)},stop:()=>{o=!1,cancelAnimationFrame(r)}}},[]);T(()=>{t.start()},e),T(()=>()=>t.stop(),[])},_r=(n=0,e=0)=>Math.abs(n-e)<.1,Ir=(n,e=1.9,t=un)=>{const o=Ee({target:n}),r=$(()=>{if(o.tween||(o.tween=o.target),o.target.every((i,l)=>o.tween[l]===i))return t(o.tween),!0;o.tween=o.target.map((i,l)=>_r(o.tween[l],i)?i:(o.tween[l]??0)+((i??0)-(o.tween[l]??0))/e||0),t(o.tween)},[]);kr(r,[n])},Fr=({canvasWidth:n,groupOnColumn:e,config:t,miniColumn:o})=>I(()=>{if(!Array.isArray(t)||n==null||n===0)return[];const r=t.map((i,l)=>({minWidth:i.minWidth,width:i.width,flex:i.flex,priority:i.priority,name:i.name,index:l,hidden:i.name===e?.name||i.disabled})).map(i=>o?{...i,hidden:o.name!==i.name}:i).sort(({index:i,priority:l},{index:a,priority:s})=>l===s?a-i:l-s);return Tt(r,n,r.length)},[n,e,t]),Or=({host:n,canvasWidth:e,columns:t})=>{const o=n.miniBreakpoint??480,r=I(()=>e<=o,[e,o]),i=I(()=>r?t?.filter(u=>u.mini!=null).sort((u,m)=>(u.mini??0)-(m.mini??0)):[],[t,r]),[l,...a]=i??[],s=!!l;return T(()=>{n.toggleAttribute("mini",s)},[s]),{isMini:s&&r,miniColumn:l,miniColumns:a}},Vr=n=>{const e=I(()=>new CSSStyleSheet,[]);return T(()=>{n.shadowRoot.adoptedStyleSheets=[...n.shadowRoot.adoptedStyleSheets,e]},[]),e},Dr=({host:n,columns:e,settings:t,setSettings:o,resizeSpeedFactor:r,sortAndGroupOptions:i})=>{const l=Sr(n),{isMini:a,miniColumn:s,miniColumns:u}=Or({host:n,canvasWidth:l,columns:e}),{groupOnColumn:m}=i,d=Fr({canvasWidth:l,groupOnColumn:m,miniColumn:s,config:t.columns}),g=Vr(n),p=I(()=>t.columns.reduce((b,x,F)=>d[F]!=null||x.name===m?.name||x.disabled?b:[...b,e.find(h=>h.name===x.name)],[]),[e,t,d]),w=Ee({columns:t.columns});return Ir(d,r,b=>{const x=wr(b,w.columns);g.replace(x)}),zr({host:n,canvasWidth:l,layout:d,setSettings:b=>o(b(t))}),{isMini:a,collapsedColumns:p,miniColumns:u}},Ar=({host:n,...e})=>{const{csvFilename:t="omnitable.csv",xlsxFilename:o="omnitable.xlsx",xlsxSheetname:r="Omnitable",topPlacement:i="top-end"}=n;return{csvFilename:t,xlsxFilename:o,xlsxSheetname:r,topPlacement:i,...e}},Er=({host:n,selectedItems:e,data:t,sortAndGroupOptions:o,collapsedColumns:r,settings:i,filterFunctions:l,settingS:a,filters:s,...u})=>{const m=t&&t.length>0&&e.length===t.length,d=b=>{b.target.checked?n.shadowRoot.querySelector("#groupedList").selectAll():n.shadowRoot.querySelector("#groupedList").deselectAll()},{groupOnColumn:g}=o,p=I(()=>[g,...r,...i.columns.filter(b=>b.disabled)].some(b=>b&&Object.keys(l).includes(b.name)),[l,i,r]),w=I(()=>({...a,collapsed:r,badge:p,filters:s}),[a,r,p,s]);return T(()=>{const b=n.shadowRoot.querySelector("#tableContent"),x=new ResizeObserver(F=>requestAnimationFrame(()=>{n.style.setProperty("--ot-height",F[0]?.contentRect.height+"px")}));return x.observe(b),()=>x.unobserve(b)},[]),{allSelected:m,onAllCheckboxChange:d,data:t,settingsConfig:w,filters:s,groupOnColumn:g,sortAndGroup:o.sortAndGroup,...u}},Lt=c`
	<svg
		viewBox="0 0 24 24"
		preserveAspectRatio="xMidYMid meet"
		focusable="false"
...
`,Tr=n=>n?"groupRow groupRow-folded":"groupRow",Lr=({item:n,index:e})=>t=>V(t?.length>0,()=>c`
	...
	`,Mr=({onCheckboxChange:n,dataIsValid:e,groupOnColumn:t})=>(o,r,{selected:i,folded:l,toggleFold:a})=>c` <div
		...
		</div>`,Nr=({host:n,error:e,dataIsValid:t,processedItems:o,columns:r,collapsedColumns:i,miniColumns:l,sortAndGroupOptions:a,rowPartFn:s,...u})=>{const{loading:m=!1,displayEmptyGroups:d=!1,compareItemsFn:g}=n,p=Te({shiftKey:!1,ctrlKey:!1}),w=$(h=>{const k=h.target.dataItem,f=h.target.checked;p.current.shiftKey?n.shadowRoot.querySelector("#groupedList").toggleSelectTo(k,f):p.current.ctrlKey?(h.target.checked=!0,n.shadowRoot.querySelector("#groupedList").selectOnly(k)):n.shadowRoot.querySelector("#groupedList").toggleSelect(k,f),h.preventDefault(),h.stopPropagation()},[]);T(()=>{const h=({shiftKey:k,ctrlKey:f})=>{p.current={shiftKey:k,ctrlKey:f}};return window.addEventListener("keydown",h),window.addEventListener("keyup",h),()=>{window.removeEventListener("keydown",h),window.removeEventListener("keyup",h)}},[]);const b=$(h=>{const k=h.composedPath();k.slice(0,k.indexOf(h.currentTarget)).find(v=>v.matches?.("a, .checkbox, .expand"))||n.dispatchEvent(new window.CustomEvent("omnitable-item-click",{bubbles:!0,composed:!0,detail:{item:h.currentTarget.dataItem,index:h.currentTarget.dataIndex}}))},[]),{groupOnColumn:x}=a,F=$((h,k)=>f=>Vn(n,h,k,f),[]);return{...u,processedItems:o,dataIsValid:t,filterIsTooStrict:t&&o.length<1,loading:m,compareItemsFn:g,displayEmptyGroups:d,error:e,renderItem:I(()=>Rr({columns:r,collapsedColumns:i,miniColumns:l,onItemClick:b,onCheckboxChange:w,dataIsValid:t,groupOnColumn:x,onItemChange:F,rowPartFn:s}),[r,i,b,w,t,x,F,s]),renderGroup:I(()=>Mr({onCheckboxChange:w,dataIsValid:t,groupOnColumn:x}),[w,t,x])}},jr=(n,e)=>{if(n===e)return 0;if(n==null)return-1;if(e==null)return 1;const t=typeof n,o=typeof e;return t==="object"&&o==="object"?n.toString()<e.toString()?-1:1:t==="number"&&o==="number"?n-e:t==="string"&&o==="string"?n<e?-1:1:t==="boolean"&&o==="boolean"?n?-1:1:(console.warn("unsupported sort",t,n,o,e),0)},Ce=(n,e)=>(t,o)=>jr(n(t),n(o))*(e?-1:1),Pr=n=>n.replace(/([a-z0-9])([A-Z])/gu,"$1-$2").toLowerCase(),mt=(n,e)=>{!n||!e||Object.entries(e).forEach(([t,o])=>{n[B].__ownChange=!0,n[B][t]=o,n[B].__ownChange=!1,n[B].dispatchEvent(new CustomEvent(`${Pr(t)}-changed`,{bubbles:!0,detail:{value:o}}))})},Se=(n,e)=>Object.assign(n,{[he]:e}),ce=Symbol("unparsed"),Br=({data:n,columns:e,hashParam:t,sortAndGroupOptions:o,noLocalSort:r,noLocalFilter:i})=>{const{groupOnColumn:l,groupOnDescending:a,sortOnColumn:s,descending:u}=o,m=$(([f,v])=>{const y=e.find(({name:z})=>z===f);return y==null?[f,void 0]:[f,v.filter&&y.serializeFilter(y,v.filter)]},[e]),d=$(([f,v])=>{const y=e.find(({name:S})=>S===f);if(y==null)return[f,{[ce]:v}];const z={filter:y.deserializeFilter(y,v)};return mt(y,z),[f,z]},[e]),[g,p]=J({},t,{multi:!0,suffix:"-filter--",write:m,read:d}),w=$((f,v)=>p(y=>{const z=be(v,y[f]);return mt(e.find(S=>S.name===f),z),{...y,[f]:{...y[f],...z}}}),[e,p]),b=I(()=>Object.values(g).map(f=>f.filter),[g]),x=I(()=>Object.fromEntries(e.map(f=>[f.name,!f.noLocalFilter&&f.getFilterFn(f,g[f.name]?.filter)]).filter(([,f])=>!!f)),[e,...b]),F=I(()=>!Array.isArray(n)||n.length===0?[]:Object.entries(x).length===0||i?n.slice():n.filter(f=>Object.values(x).every(v=>v(f))),[n,x,i]),h=I(()=>{if(!r&&!l&&s!=null&&s.sortOn!=null)return F.slice().sort(Ce(f=>s.getComparableValue({...s,valuePath:s.sortOn},f),u));if(l!=null&&l.groupOn!=null){const f=F.reduce((v,y)=>{const z=l.getComparableValue({...l,valuePath:l.groupOn},y);if(z===void 0)return v;let S=v.find(R=>R.id===z);return S?(S.items.push(y),v):(S={id:z,name:z,items:[y]},[...v,S])},[]);return f.sort(Ce(v=>l.getComparableValue({...l,valuePath:l.groupOn},v.items[0]),a)),!s||r?f:f.filter(v=>Array.isArray(v.items)).map(v=>(v.items.sort(Ce(y=>s.getComparableValue({...s,valuePath:s.sortOn},y),u)),v))}return F},[F,l,a,s,u,r]),k=I(()=>{let f=0,v=0;const y=[];return h.forEach(z=>{if(Array.isArray(z.items)){Se(z,v++),z.items.forEach(S=>{Se(S,f++),y.push(S)});return}return Se(z,f++),y.push(z)},[]),y},[h]);return T(()=>{p(f=>Object.values(f).some(y=>y[ce]!=null)?Object.fromEntries(Object.entries(f).map(([y,z])=>z[ce]==null?[y,z]:d([y,z[ce]]))):f)},[d]),{processedItems:h,visibleData:k,filters:g,filterFunctions:x,setFilterState:w}},Hr=n=>{const e=o=>{const r=n.data.indexOf(o);if(r<0)return null;const i=n.data.splice(r,1);if(n.data=n.data.slice(),Array.isArray(i)&&i.length>0)return i[0]},t=(o,r)=>{n.data.splice(o,1,r),n.data=n.data.slice()};return{removeItem:e,removeItems(o){const r=[];for(let i=o.length-1;i>=0;i-=1){const l=n.removeItem(o[i]);l!=null&&r.push(l)}return r},replaceItemAtIndex:t,replaceItem(o,r){const i=n.data.indexOf(o);if(i>-1)return t(i,r)},selectItem(o){n.shadowRoot.querySelector("#groupedList").select(o)},selectAll(){n.shadowRoot.querySelector("#groupedList").selectAll()},deselectAll(){n.shadowRoot.querySelector("#groupedList").deselectAll()},deselectItem(o){n.shadowRoot.querySelector("#groupedList").deselect(o)},isItemSelected(o){return n.shadowRoot.querySelector("#groupedList").isItemSelected(o)}}},Gr=({host:n,visibleData:e,filters:t,...o})=>{const{setFilterState:r}=o,i=I(()=>Hr(n),[]);Ie(o,Object.values(o)),Ie(i,Object.values(i)),T(()=>{const a=s=>r(s.detail.name,u=>({...u,...s.detail.state}));return n.addEventListener("legacy-filter-changed",a),()=>n.removeEventListener("legacy-filter-changed",a)},[]),P("visibleData",e),P("sortedFilteredGroupedItems",o.sortedFilteredGroupedItems),P("selectedItems",o.selectedItems),P("sortOn",o.sortOn),P("descending",o.descending),P("isMini",o.isMini);const l=I(()=>Object.fromEntries(Object.entries(t).filter(([,{filter:a}])=>a!==void 0).map(([a,{filter:s}])=>[a,s])),[t]);P("filters",l,Object.values(l))},Wr=n=>{const{hashParam:e,settingsId:t,data:o,resizeSpeedFactor:r,noLocal:i,noLocalSort:l=i,noLocalFilter:a=i,error:s,rowPartFn:u}=n,m=xr({settingsId:t,host:n}),{settings:d,setSettings:g,columns:p,resetRef:w}=m,b=Tn(p,e,d,g,w),{processedItems:x,visibleData:F,filters:h,setFilterState:k,filterFunctions:f}=Br({data:o,columns:p,hashParam:e,sortAndGroupOptions:b,noLocalSort:l,noLocalFilter:a}),{isMini:v,collapsedColumns:y,miniColumns:z}=Dr({host:n,columns:p,settings:d,setSettings:g,resizeSpeedFactor:r,sortAndGroupOptions:b}),S=o&&Array.isArray(o)&&o.length>0,[R,G]=M([]);return Gr({host:n,visibleData:F,sortedFilteredGroupedItems:x,columns:p,filters:h,setFilterState:k,selectedItems:R,isMini:v,...b}),{header:Er({host:n,selectedItems:R,sortAndGroupOptions:b,dataIsValid:S,data:o,columns:p,filters:h,collapsedColumns:y,settings:d,filterFunctions:f,settingS:m,setFilterState:k,hideSelectAll:n.hideSelectAll===!0}),list:Nr({host:n,error:s,dataIsValid:S,processedItems:x,setSelectedItems:G,columns:p,collapsedColumns:y,miniColumns:z,sortAndGroupOptions:b,rowPartFn:u}),footer:Ar({host:n,selectedItems:R,columns:p})}};customElements.define("cosmoz-grouped-list-row",class extends HTMLElement{get item(){return this._item}set item(e){this._item=e,this._render()}get index(){return this._index}set index(e){this._index=e,this._render()}get renderFn(){return this._renderFn}set renderFn(e){this._renderFn=e,this._render()}_render(){this._item==null||this._index==null||this._renderFn==null||ke(this._renderFn(this._item,this._index),this)}});const qr={group:Symbol("group")},ve=(n,e)=>(e.has(n)||e.set(n,{}),e.get(n)),Xr=(n,e)=>!!ve(n,e).expanded,Ur=(n,e)=>!!ve(n,e).folded,Fe=n=>n?n.items instanceof Array:!1,Kr=n=>{if(!Array.isArray(n)||n.length===0)return;const e=Array.isArray(n[0]?.items);if(!n.every(o=>Array.isArray(o.items)===e))throw new Error("Data must be homogeneous.")},Jr=(n,e,t)=>Array.isArray(n)?(Kr(n),n.reduce((r,i)=>{const l=i;return l.items?l.items.length?ve(i,t).folded?r.concat(i):r.concat(i,l.items.map(a=>Object.assign(a,{[qr.group]:i}))):e?r.concat(i):r:r.concat(i)},[])):void 0,Qr=(n,...e)=>typeof n=="function"?n(...e):n,Yr=(n,e)=>n===e,Zr=()=>{const[n,e]=M(()=>[new WeakMap]);return{setItemState:$((o,r)=>e(([i])=>{const l=ve(o,i);return Object.assign(l,Qr(r,l)),[i]}),[]),state:n[0],signal:n}},ei=()=>{const{setItemState:n,state:e,signal:t}=Zr(),o=$((i,l)=>{Fe(i)&&n(i,a=>({folded:l!==void 0?l:!a.folded}))},[]),r=$((i,l)=>{Fe(i)||n(i,a=>({expanded:l!==void 0?!l:!a.expanded}))},[]);return{state:e,signal:t,toggleFold:o,toggleCollapse:r}},ti=({initial:n,compareItemsFn:e,data:t,flatData:o})=>{const[r,i]=M(n),[l,a]=M(),s=$(h=>r.includes(h),[r]),u=$(h=>h?.items?.every(s)??!1,[s]),m=$(h=>s(h)||u(h),[s,u]),d=$(h=>{const f=h.items??[h];i(v=>[...v,...f.filter(y=>!v.includes(y))]),a(h)},[]),g=$(h=>{const f=h.items??[h];i(v=>v.filter(y=>!f.includes(y))),a(h)},[]),p=$(h=>{i(h.items?.slice()||[h]),a(h)},[]),w=$(()=>{i(t.flatMap(h=>h.items||h)),a(void 0)},[t]),b=$(()=>{i([]),a(void 0)},[]),x=$((h,k=!m(h))=>k?d(h):g(h),[m]),F=$((h,k)=>{if(!o)return;const f=l?o.findIndex(z=>e(z,l)):-1;if(f<0)return x(h,k);const[v,y]=[f,o.indexOf(h)].sort((z,S)=>z-S);o.slice(v,y+1).forEach((z,S,R)=>{S>0&&S<R.length-1&&Fe(z)||x(z,k)}),a(h)},[o,e,x]);return T(()=>i(h=>h.length>0&&o?o.filter(k=>h.find(f=>e(k,f))):h),[o]),{selectedItems:r,isItemSelected:s,isGroupSelected:u,isSelected:m,select:d,deselect:g,selectOnly:p,selectAll:w,deselectAll:b,toggleSelect:x,toggleSelectTo:F}},ni={host:{position:"relative",display:"flex",flexDirection:"column"}},oi=n=>{const{data:e,renderItem:t,renderGroup:o,displayEmptyGroups:r,compareItemsFn:i=Yr}=n,{toggleFold:l,toggleCollapse:a,state:s,signal:u}=ei(),m=I(()=>Jr(e,r,s),[e,r,u]),{selectedItems:d,isItemSelected:g,isGroupSelected:p,isSelected:w,select:b,deselect:x,selectOnly:F,selectAll:h,deselectAll:k,toggleSelect:f,toggleSelectTo:v}=ti({initial:[],compareItemsFn:i,data:e,flatData:m}),y=$((S,R)=>Array.isArray(S.items)?o(S,R,{selected:p(S),folded:Ur(S,s),toggleSelect:G=>f(S,typeof G=="boolean"?G:void 0),toggleFold:()=>l(S)}):t(S,R,{selected:d.includes(S),expanded:Xr(S,s),toggleSelect:G=>f(S,typeof G=="boolean"?G:void 0),toggleCollapse:()=>a(S)}),[t,o,d,f,u]);ht(()=>{Object.assign(n.style,ni.host)},[]),P("selectedItems",d);const z={toggleFold:l,toggleCollapse:a,isItemSelected:g,isGroupSelected:p,isSelected:w,select:b,deselect:x,selectOnly:F,selectAll:h,deselectAll:k,toggleSelect:f,toggleSelectTo:v};return Ie(z,Object.values(z)),{renderRow:y,flatData:m}},ri=({renderRow:n,flatData:e})=>cn({items:e,renderItem:(t,o)=>c`<cosmoz-grouped-list-row
				.item=${t}
				.index=${o}
				.renderFn=${n}
