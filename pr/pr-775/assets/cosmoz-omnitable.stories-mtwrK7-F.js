import{x as c,B as ke,E as ht}from"./iframe-BSi74Mbm.js";import{t as K,c as O,g as C,P as I,u as T,a as xt,m as yt,s as Nt,D as Rt,b as Bt,e as Gt,i as $e,h as Oe,d as Ht,f as vt,r as jt,j as Wt,k as A,l as Ut,n as we,p as he,o as k,q as Kt,v as Ce,w as Jt,x as Xt,y as Qt,T as Yt,z as M,A as Zt,B as ei,C as ti,E as ii,F as $,G as ai,H as ni,I as $t,J as Le,K as F,L as wt,M as De,N as oi,O as Ct,Q as ri,R as Te,S as Ye,U as li,V as Ve,W as si,X as ui,Y as Ee,Z as Pe,_ as di,$ as Ie,a0 as N,a1 as ci,a2 as ze,a3 as mi}from"./cosmoz-collapse-BwDFl84t.js";import"./preload-helper-PPVm8Dsz.js";const pi=K`
	:host {
		display: flex;
	}
	:host(:not([light])) {
...
`,Li=i=>{const{label:e,value:t,slot:a}=i,n=c`<div
	...
	>
		${t||""}
	</div>`;return jt(n,{label:e})};customElements.define("cosmoz-omnitable-dropdown-input",O(Li,{observedAttributes:Mi,styleSheets:[Oi]}));const ne=({title:i,tooltip:e="",filterText:t="",onOpenedChanged:a,content:n,horizontalAlign:o="left",externalValues:r=null,headerFocused:l})=>{const u={filtered:!!t,...r!=null&&{[`external-values-${r}`]:!0}};return c`
		<style>
			.dropdown {
				outline: none;
			}
...
`,r=e.map(l=>i.map(u=>{const s=u.getString(u,l);return s==null?"":gt(String(s))}).join(";")+`
`);r.unshift(o),Ct(new File(r,t,{type:"text/csv;charset=utf-8"}))},Ka=(i,e)=>{const t=i.map(n=>n.title),a=e.map(n=>i.map(o=>{const r=o.toXlsxValue(o,n);return r??""}));return a.unshift(t),a},Ja=(i,e,t,a)=>{const n=Ka(i,e),o=new ri(t).addSheetFromData(n,a).generate();Ct(new File([o],t,{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}))},Xa=({columns:i,selectedItems:e,csvFilename:t,xlsxFilename:a,xlsxSheetname:n,topPlacement:o})=>c`<cosmoz-bottom-bar
	...
	</cosmoz-bottom-bar>`,Qa=({allSelected:i,onAllCheckboxChange:e,sortAndGroup:t,dataIsValid:a,data:n,columns:o,filters:r,groupOnColumn:l,setFilterState:u,settingsConfig:s,hideSelectAll:d})=>c`<sort-and-group-provider .value=${t}>
...
`,Za=({settingsConfig:i})=>{const{columns:e,collapsed:t}=i,a=e.filter(n=>!t.some(o=>o.name===n.name));return c`<div class="skeleton">
	...
	</div>`};customElements.define("cosmoz-omnitable-skeleton",O(Za,{styleSheets:[Ya]}));const en=(i,e)=>{const{settingsConfig:t}=i,{processedItems:a,dataIsValid:n,filterIsTooStrict:o,loading:r,displayEmptyGroups:l,compareItemsFn:u,setSelectedItems:s,renderItem:d,renderGroup:m,error:b}=e;return c`${k(!r&&!n&&!b,()=>c`<div class="tableContent-empty">
...
`,pe=i=>{const e=parseInt(i,10);return isFinite(e)?e:void 0},nn=i=>{const{config:e}=i,{settings:t,setSettings:a,collapsed:n}=e,o=Ve({collapsed:n,settings:t.columns,setSettings:$(r=>a(l=>({...l,columns:r})),[a])});return{...e,onDown:$(r=>{r.target.closest(".pull")&&(o.handle=r.currentTarget)},[o]),onDragStart:$(r=>{const{target:l}=r,u=pe(l.dataset.index);if(!o.handle?.contains(l)||u==null)return r.preventDefault();o.handle=null,r.dataTransfer.effectAllowed="move",r.dataTransfer.setData("omnitable/sort-index",u),r.dataTransfer.setData("text/plain",u),setTimeout(()=>l.classList.add("drag"),0),l.addEventListener("dragend",s=>s.target.classList.remove("drag"),{once:!0})},[o]),onDragEnter:$(r=>{const l=r.currentTarget;l===r.target&&(r.preventDefault(),r.dataTransfer.dropEffect="move",l.classList.add("dragover"))},[]),onDragOver:$(r=>{r.preventDefault(),r.currentTarget.classList.add("dragover")},[]),onDragLeave:$(r=>{const l=r.currentTarget;l.contains(r.relatedTarget)||l.classList.remove("dragover")},[]),onDrop:$(r=>{const l=pe(r.dataTransfer.getData("omnitable/sort-index")),u=pe(r.currentTarget.dataset.index),{settings:s,setSettings:d}=o;r.currentTarget.classList.remove("dragover"),r.preventDefault();const m=s.slice();m.splice(u+(l>=u?0:-1),0,m.splice(l,1)[0]),d(m)},[o]),onToggle:$(r=>{const{settings:l,setSettings:u}=o,s=l.map(m=>({...m,disabled:m.disabled||o.collapsed?.some(b=>b.name===m.name)})),d=pe(r.target.closest("[data-index]")?.dataset.index);s.splice(d,1,{...l[d],disabled:!r.target.checked,priority:r.target.checked?l.reduce((m,b)=>Math.max(m,b.priority),0)+1:l[d].priority}),u(s)},[o])}},on=[si({apply({availableHeight:i,elements:e}){Object.assign(e.floating.style,{maxHeight:`${Math.max(0,i)}px`})}}),...ui],rn=({onDragStart:i,onDragEnter:e,onDragOver:t,onDragLeave:a,onDrop:n,onDown:o,onToggle:r,collapsed:l,filters:u})=>(s,d)=>{const m=!!l?.find(p=>p.name===s.name),b=!s.disabled&&!m;return c` <div
		...
		>
			...
			<input
				...
				.indeterminate=${m}
			/>
		</div>`},ln=i=>{const{settings:e,settingsId:t,onSave:a,onReset:n,hasChanges:o,opened:r,setOpened:l,...u}=nn(i);return c` <div class="headline">
			${A("Sort and filter")}
			<button
				class="close"
...
`;customElements.define("cosmoz-omnitable-settings",O(sn,{styleSheets:[De(an)]}));const xe=["sortOn","descending","groupOn","groupOnDescending"],ge=i=>e=>typeof e=="object"&&e!==null&&"name"in e&&e.name===i,un=(i=[],e=[],t=[])=>{const a=e.filter(r=>i.some(ge(r.name))),n=i.filter(r=>r.name!=null&&!e.some(ge(r.name))&&!t.some(ge(r.name))),o=t.filter(r=>!e.some(ge(r.name)));return[...a,...o.flatMap(r=>{const l=i.find(u=>u.name===r.name);return l?{...r,title:l.title??r.title??"",minWidth:parseInt(l.minWidth??"0",10)}:[]}),...n.map(r=>{const{name:l,title:u,priority:s,minWidth:d,width:m,flex:b}=r;return{name:l??"",title:u??"",priority:s??0,minWidth:parseInt(d??"0",10),width:parseInt(m??"0",10),flex:parseInt(b??"0",10)}})]},dn=(i,e)=>({...e,...Ee(Array.from(xe))(i),columns:i.columns?.map(Ee(["name","priority","width","flex","disabled"]))}),cn=({columns:i,settings:e,savedSettings:t,initial:a})=>({...Object.fromEntries(xe.flatMap(o=>a?.[o]!=null?[[o,a[o]]]:[])),...t?Ee(Array.from(xe))(t):{},...e,columns:un(i,e?.columns,t?.columns)}),mn=({prefix:i="omnitable-"}={})=>({write:async(t,a)=>{const n=i+t;try{a?localStorage.setItem(n,JSON.stringify(a)):localStorage.removeItem(n)}catch(o){console.error(o)}},read:async t=>{if(t)try{const a=localStorage.getItem(i+t);return a==null?void 0:JSON.parse(a)}catch(a){console.error(a)}}}),pn=$t(mn),gn=()=>{const i=Le(pn);return F(()=>i(),[i])},bn=(i,e,t,a)=>{const[n,o]=T(),{read:r,write:l}=gn();return M(async()=>{i&&o(await r(i))},[i,r]),{settingsId:i,savedSettings:n,onSave:$(async()=>{if(!i)return;const u=dn(e,n);await l(i,u),t(),o(u)},[e,n]),onReset:$(async u=>{t(),u.shiftKey&&(await l(i),o()),a?.()},[a]),hasChanges:e!=null}},fn=({settingsId:i,host:e})=>{const t=F(()=>Object.fromEntries(xe.map(h=>[h,e[h]])),[]),a=Pe(),n=$(()=>{a.current?.(t)},[t]),[o,r]=T(),[l,u]=T({columns:!0,sort:!0}),{savedSettings:s,...d}=bn(i,o,r,n),{enabledColumns:m}=e,b=Ei(e,{enabledColumns:m}),p=F(()=>cn({columns:b,settings:o,savedSettings:s,initial:t}),[b,o,s]),z=F(()=>p.columns.map(h=>b.find(v=>v.name===h.name)).filter(Boolean),[b,...p.columns.map(h=>h.name)]);return{...d,opened:l,setOpened:u,settings:p,columns:z,setSettings:r,resetRef:a}},bt=i=>Number.isFinite(i)?i:0,hn=(i,e)=>{const t=[];let[a,n]=i.reduce(([d,m],{width:b,flex:p})=>[d+b,m+p],[0,0]),o=e-a,r=bt(o/n),l=0,u=0,s=0;for(let d=0;d<i.length;d++){const{width:m,minWidth:b,flex:p}=i[d],z=o>=0?r*p:m*o/a;if(b>m+z){l+=m,u+=b,s+=p,t[d]=b;continue}if(p===0){l+=m,u+=m,t[d]=m;continue}}a-=l,o=e-u-a,n-=s,r=bt(o/n);for(let d=0;d<i.length;d++){if(t[d]!=null)continue;const{width:m,flex:b}=i[d],p=o>=0?r*b:m*o/a;t[d]=m+p}return t},ye=Symbol("index"),xn=(i,e)=>{if(typeof i.findLastIndex=="function")return i.findLastIndex(e);for(let t=i.length-1;t>=0;t--)if(e(i[t],t,i))return t;return-1},yn=(i,e)=>{const t=xn(i,r=>r!=null&&r>0),a=(r,l)=>`.cell[name="${r}"], cosmoz-omnitable-skeleton::part(cell-${r}){width: ${Math.floor(l)}px;padding: 0 min(3px, ${l/2}px)}`,n=r=>`cosmoz-omnitable-resize-nub[name="${r}"]{display:none}`,o=r=>`cosmoz-omnitable-resize-nub[name="${r}"], .cell[name="${r}"]{display:none}`;return e.map((r,l)=>{const u=i[l];if(u==null||u===0)return o(r.name);const s=a(r.name,u);return l===t?`${s}
${n(r.name)}`:s}).join(`
`)},Vt=(i,e,t)=>{const a=i.filter(l=>!l.hidden),n=a.reduce((l,{width:u})=>l+u,0);if(a.length>1&&n>e)return Vt(a.slice(1),e,t);const o=a.reduce(([l,u],s,d)=>[Math.max(l,s.index),s.index>l?d:u],[-1,-1])[1];return o!==-1&&(a[o].flex=1),hn(a,e).reduce((l,u,s)=>(l[a[s].index]=u,l),new Array(t).fill(void 0))},vn=(i,e)=>i.length===0?".cell {display: none;}":yn(i,e),$n=({host:i,canvasWidth:e,layout:t,setSettings:a})=>{const n=Pe();n.current=o=>a(r=>{const l=r.columns,{detail:{newWidth:u,column:s}}=o,d=l.findIndex(p=>p.name===s.name),m=[],b=l.reduce((p,z)=>Math.max(p,z.priority),-1/0);for(let p=0;p<t.length;p++)if(m[p]={...l[p]},p<d&&t[p]&&(m[p].width=t[p],m[p].flex=0,m[p].priority=b),p===d){const z=t.reduce((h,v,_)=>_<d&&v?h-v:h,e);m[p].width=Math.min(z,Math.max(u,l[p].minWidth)),m[p].flex=0,m[p].priority=b}return{...r,columns:m}}),M(()=>{const o=r=>n.current?.(r);return i.addEventListener("column-resize",o),()=>i.removeEventListener("column-resize",o)},[])},wn=(i,e)=>M(()=>{const t=([n])=>{n.contentRect?.width!==0&&requestAnimationFrame(()=>e(n.contentRect?.width-20-44-24))},a=new ResizeObserver(t);return a.observe(i),()=>a.unobserve(i)},[]),Cn=i=>{const[e,t]=T(()=>i.getBoundingClientRect().width);return wn(i,t),e},Sn=(i,e)=>{const t=F(()=>{let a=!1,n;const o=()=>{if(!a)return;n=requestAnimationFrame(o),i()&&(a=!1)};return{start:()=>{a=!0,cancelAnimationFrame(n),n=requestAnimationFrame(o)},stop:()=>{a=!1,cancelAnimationFrame(n)}}},[]);M(()=>{t.start()},e),M(()=>()=>t.stop(),[])},zn=(i=0,e=0)=>Math.abs(i-e)<.1,qn=(i,e=1.9,t=di)=>{const a=Ve({target:i}),n=$(()=>{if(a.tween||(a.tween=a.target),a.target.every((o,r)=>a.tween[r]===o))return t(a.tween),!0;a.tween=a.target.map((o,r)=>zn(a.tween[r],o)?o:(a.tween[r]??0)+((o??0)-(a.tween[r]??0))/e||0),t(a.tween)},[]);Sn(n,[i])},An=({canvasWidth:i,groupOnColumn:e,config:t,miniColumn:a})=>F(()=>{if(!Array.isArray(t)||i==null||i===0)return[];const n=t.map((o,r)=>({minWidth:o.minWidth,width:o.width,flex:o.flex,priority:o.priority,name:o.name,index:r,hidden:o.name===e?.name||o.disabled})).map(o=>a?{...o,hidden:a.name!==o.name}:o).sort(({index:o,priority:r},{index:l,priority:u})=>r===u?l-o:r-u);return Vt(n,i,n.length)},[i,e,t]),Fn=({host:i,canvasWidth:e,columns:t})=>{const a=i.miniBreakpoint??480,n=F(()=>e<=a,[e,a]),o=F(()=>n?t?.filter(s=>s.mini!=null).sort((s,d)=>(s.mini??0)-(d.mini??0)):[],[t,n]),[r,...l]=o??[],u=!!r;return M(()=>{i.toggleAttribute("mini",u)},[u]),{isMini:u&&n,miniColumn:r,miniColumns:l}},_n=i=>{const e=F(()=>new CSSStyleSheet,[]);return M(()=>{i.shadowRoot.adoptedStyleSheets=[...i.shadowRoot.adoptedStyleSheets,e]},[]),e},kn=({host:i,columns:e,settings:t,setSettings:a,resizeSpeedFactor:n,sortAndGroupOptions:o})=>{const r=Cn(i),{isMini:l,miniColumn:u,miniColumns:s}=Fn({host:i,canvasWidth:r,columns:e}),{groupOnColumn:d}=o,m=An({canvasWidth:r,groupOnColumn:d,miniColumn:u,config:t.columns}),b=_n(i),p=F(()=>t.columns.reduce((h,v,_)=>m[_]!=null||v.name===d?.name||v.disabled?h:[...h,e.find(g=>g.name===v.name)],[]),[e,t,m]),z=Ve({columns:t.columns});return qn(m,n,h=>{const v=vn(h,z.columns);b.replace(v)}),$n({host:i,canvasWidth:r,layout:m,setSettings:h=>a(h(t))}),{isMini:l,collapsedColumns:p,miniColumns:s}},En=({host:i,...e})=>{const{csvFilename:t="omnitable.csv",xlsxFilename:a="omnitable.xlsx",xlsxSheetname:n="Omnitable",topPlacement:o="top-end"}=i;return{csvFilename:t,xlsxFilename:a,xlsxSheetname:n,topPlacement:o,...e}},In=({host:i,selectedItems:e,data:t,sortAndGroupOptions:a,collapsedColumns:n,settings:o,filterFunctions:r,settingS:l,filters:u,...s})=>{const d=t&&t.length>0&&e.length===t.length,m=h=>{h.target.checked?i.shadowRoot.querySelector("#groupedList").selectAll():i.shadowRoot.querySelector("#groupedList").deselectAll()},{groupOnColumn:b}=a,p=F(()=>[b,...n,...o.columns.filter(h=>h.disabled)].some(h=>h&&Object.keys(r).includes(h.name)),[r,o,n]),z=F(()=>({...l,collapsed:n,badge:p,filters:u}),[l,n,p,u]);return M(()=>{const h=i.shadowRoot.querySelector("#tableContent"),v=new ResizeObserver(_=>requestAnimationFrame(()=>{i.style.setProperty("--ot-height",_[0]?.contentRect.height+"px")}));return v.observe(h),()=>v.unobserve(h)},[]),{allSelected:d,onAllCheckboxChange:m,data:t,settingsConfig:z,filters:u,groupOnColumn:b,sortAndGroup:a.sortAndGroup,...s}},Pt=c`
	<svg
		viewBox="0 0 24 24"
		preserveAspectRatio="xMidYMid meet"
		focusable="false"
...
`,Mn=i=>i?"groupRow groupRow-folded":"groupRow",On=({item:i,index:e})=>t=>k(t?.length>0,()=>c`
			...
			`),Ln=({columns:i,collapsedColumns:e,miniColumns:t,onItemClick:a,onCheckboxChange:n,dataIsValid:o,groupOnColumn:r,onItemChange:l,rowPartFn:u})=>(s,d,{selected:m,expanded:b,toggleCollapse:p})=>c`
		<div
			?selected=${m}
			part="${["itemRow",`itemRow-${s[ye]}`,u?.(s,d)].filter(Boolean).join(" ")}"
			.dataIndex=${s[ye]}
			.dataItem=${s}
			class="itemRow"
			@click=${a}
