...
`,se=n=>{const e=parseInt(n,10);return isFinite(e)?e:void 0},lr=n=>{const{config:e}=n,{settings:t,setSettings:o,collapsed:r}=e,i=Re({collapsed:r,settings:t.columns,setSettings:$(a=>o(l=>({...l,columns:a})),[o])});return{...e,onDown:$(a=>{a.target.closest(".pull")&&(i.handle=a.currentTarget)},[i]),onDragStart:$(a=>{const{target:l}=a,s=se(l.dataset.index);if(!i.handle?.contains(l)||s==null)return a.preventDefault();i.handle=null,a.dataTransfer.effectAllowed="move",a.dataTransfer.setData("omnitable/sort-index",s),a.dataTransfer.setData("text/plain",s),setTimeout(()=>l.classList.add("drag"),0),l.addEventListener("dragend",u=>u.target.classList.remove("drag"),{once:!0})},[i]),onDragEnter:$(a=>{const l=a.currentTarget;l===a.target&&(a.preventDefault(),a.dataTransfer.dropEffect="move",l.classList.add("dragover"))},[]),onDragOver:$(a=>{a.preventDefault(),a.currentTarget.classList.add("dragover")},[]),onDragLeave:$(a=>{const l=a.currentTarget;l.contains(a.relatedTarget)||l.classList.remove("dragover")},[]),onDrop:$(a=>{const l=se(a.dataTransfer.getData("omnitable/sort-index")),s=se(a.currentTarget.dataset.index),{settings:u,setSettings:c}=i;a.currentTarget.classList.remove("dragover"),a.preventDefault();const m=u.slice();m.splice(s+(l>=s?0:-1),0,m.splice(l,1)[0]),c(m)},[i]),onToggle:$(a=>{const{settings:l,setSettings:s}=i,u=l.map(m=>({...m,disabled:m.disabled||i.collapsed?.some(h=>h.name===m.name)})),c=se(a.target.closest("[data-index]")?.dataset.index);u.splice(c,1,{...l[c],disabled:!a.target.checked,priority:a.target.checked?l.reduce((m,h)=>Math.max(m,h.priority),0)+1:l[c].priority}),s(u)},[i])}},sr=[cn({apply({availableHeight:n,elements:e}){Object.assign(e.floating.style,{maxHeight:`${Math.max(0,n)}px`})}}),...dn],ur=({onDragStart:n,onDragEnter:e,onDragOver:t,onDragLeave:o,onDrop:r,onDown:i,onToggle:a,collapsed:l,filters:s})=>(u,c)=>{const m=!!l?.find(p=>p.name===u.name),h=!u.disabled&&!m;return d` <div
			class="item"
			data-index=${c}
			@mousedown=${i}
			draggable="true"
