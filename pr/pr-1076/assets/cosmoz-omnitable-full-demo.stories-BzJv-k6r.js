...
`,de=n=>{const e=parseInt(n??"",10);return isFinite(e)?e:void 0},br=n=>{const{config:e}=n,{settings:t,setSettings:o,collapsed:r,requestTween:i}=e,l=ke({collapsed:r,settings:t.columns,requestTween:i,setSettings:S(a=>o(s=>({...s,columns:a})),[o])});return{...e,onDown:S(a=>{!a.target||!a.target.closest(".pull")||(l.handle=a.currentTarget)},[l]),onDragStart:S(a=>{const{target:s}=a,u=de(s?.dataset.index);if(!l.handle?.contains(s)||u==null)return a.preventDefault();l.handle=null,a.dataTransfer.effectAllowed="move",a.dataTransfer.setData("omnitable/sort-index",String(u)),a.dataTransfer.setData("text/plain",String(u)),setTimeout(()=>s.classList.add("drag"),0),s.addEventListener("dragend",m=>m.target.classList.remove("drag"),{once:!0})},[l]),onDragEnter:S(a=>{const s=a.currentTarget;s===a.target&&(a.preventDefault(),a.dataTransfer.dropEffect="move",s.classList.add("dragover"))},[]),onDragOver:S(a=>{a.preventDefault(),a.currentTarget.classList.add("dragover")},[]),onDragLeave:S(a=>{const s=a.currentTarget;s.contains(a.relatedTarget)||s.classList.remove("dragover")},[]),onDrop:S(a=>{const s=de(a.dataTransfer?.getData("omnitable/sort-index")),u=de(a.currentTarget.dataset.index),{settings:m,setSettings:d,requestTween:g}=l;if(a.currentTarget.classList.remove("dragover"),a.preventDefault(),s==null||u==null)return;const p=m.slice();p.splice(u+(s>=u?0:-1),0,p.splice(s,1)[0]),g?.(),d(p)},[l]),onToggle:S(a=>{const{settings:s,setSettings:u,requestTween:m}=l,d=s.map(h=>({...h,disabled:h.disabled||l.collapsed?.some(v=>v.name===h.name)})),g=de(a.target?.closest("[data-index]")?.dataset.index),p=a.target;g!=null&&(d.splice(g,1,{...s[g],disabled:!p?.checked,priority:p?.checked?s.reduce((h,v)=>Math.max(h,v.priority??0),0)+1:s[g]?.priority}),m?.(),u(d))},[l])}},xr=[gn({apply({availableHeight:n,elements:e}){Object.assign(e.floating.style,{maxHeight:`${Math.max(0,n)}px`})}}),...pn],vr=({onDragStart:n,onDragEnter:e,onDragOver:t,onDragLeave:o,onDrop:r,onDown:i,onToggle:l,collapsed:a,filters:s})=>(u,m)=>{const d=!!a?.find(p=>p.name===u.name),g=!u.disabled&&!d;return c` <div
			class="item"
			data-index=${m}
			@mousedown=${i}
			draggable="true"
