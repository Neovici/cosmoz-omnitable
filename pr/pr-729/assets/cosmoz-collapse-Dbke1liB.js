import{f as Eo,B as Nl,x as O,E as we,T as Y,m as Mo,i as Vl,p as Rl,v as Oe,s as ht,M as cn}from"./iframe-t8JveX5x.js";import{_ as Dl}from"./preload-helper-PPVm8Dsz.js";/**
 * @license
 ...
 */const Y1=t=>t,J1=(t,e)=>O`${e}: ${JSON.stringify(t,null,2)}`;class X1 extends Wi{constructor(e){if(super(e),this._virtualizer=null,this._first=0,this._last=-1,this._renderItem=(i,n)=>J1(i,n+this._first),this._keyFunction=(i,n)=>Y1(i,n+this._first),this._items=[],e.type!==re.CHILD)throw new Error("The virtualize directive can only be used in child expressions")}render(e){e&&this._setFunctions(e);const i=[];if(this._first>=0&&this._last>=this._first)for(let n=this._first;n<=this._last;n++)i.push(this._items[n]);return F1(i,this._keyFunction,this._renderItem)}update(e,[i]){this._setFunctions(i);const n=this._items!==i.items;return this._items=i.items||[],this._virtualizer?this._updateVirtualizerConfig(e,i):this._initialize(e,i),n?Y:this.render()}async _updateVirtualizerConfig(e,i){if(!await this._virtualizer.updateLayoutConfig(i.layout||{})){const s=e.parentNode;this._makeVirtualizer(s,i)}this._virtualizer.items=this._items}_setFunctions(e){const{renderItem:i,keyFunction:n}=e;i&&(this._renderItem=(s,r)=>i(s,r+this._first)),n&&(this._keyFunction=(s,r)=>n(s,r+this._first))}_makeVirtualizer(e,i){this._virtualizer&&this._virtualizer.disconnected();const{layout:n,scroller:s,items:r}=i;this._virtualizer=new U1({hostElement:e,layout:n,scroller:s}),this._virtualizer.items=r,this._virtualizer.connected()}_initialize(e,i){const n=e.parentNode;n&&n.nodeType===1&&(n.addEventListener("rangeChanged",s=>{this._first=s.first,this._last=s.last,this.setValue(this.render())}),this._makeVirtualizer(n,i))}disconnected(){this._virtualizer?.disconnected()}reconnected(){this._virtualizer?.connected()}}const G1=me(X1),Z1=(t,e)=>{if(!t||!e)return;const i=Object.keys(e);return Object.fromEntries(Object.keys(t).flatMap(n=>i.includes(n)?[]:[[n,void 0]]))};class Q1 extends Ue{_props;render(e){return Y}update(e,[i]){return this._props!==i&&Object.assign(e.element,Z1(this._props,i),this._props=i),Y}}const ef=me(Q1),tf=t=>{const e=qt(),i=I(()=>new CSSStyleSheet,[]);k(()=>{e.shadowRoot.adoptedStyleSheets=[...e.shadowRoot.adoptedStyleSheets,i]},[]),k(()=>{i.replaceSync(t)},[t])};function vl(t){return t?e=>e?.[t]:it}const nf=t=>{const e=vl(t);return i=>typeof i=="string"?i:e(i)?.toString()||""},sf=t=>e=>{const i={};for(const n in e)t.includes(n)&&(i[n]=e[n]);return i},rf="data:image/svg+xml,%3Csvg width='11' height='8' viewBox='0 0 11 8' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M9.5 1L5.20039 7.04766L1.66348 3.46152' stroke='white' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E",of=xe`
	...
	:host([multi]) .item[aria-selected]::before {
		border-color: #5881f6;
		/* prettier-ignore */
		background: url("${rf}") #5881f6 no-repeat 50%;
	}
	:host([multi]) .sizer {
		padding-left: 33px;
	}
...
`,af=({index:t,itemHeight:e,auto:i})=>xe`
	${Ee(!i,()=>xe`
			.item {
				line-height: ${e}px;
				height: ${e}px;
...
`,lf=t=>typeof t=="object"&&t!==null&&Symbol.iterator in t;function je(t){return t==null?[]:Array.isArray(t)?t:typeof t=="string"?[t]:lf(t)?Array.from(t):[t]}const Cn=(t,e=it)=>i=>{const n=je(t).map(e);return je(i).filter(s=>!n.includes(e(s)))},cf=(t,e)=>e?i=>je(t).find(n=>n[e]===i[e]):i=>je(t).includes(i),hf=(t,e)=>{if(!e||!t)return t;const i=t.toLowerCase().indexOf(e.toLowerCase());if(i<0)return t;const n=i+e.length;return[t.slice(0,i),O`<mark>${t.slice(i,n)}</mark>`,t.slice(n)]},df=t=>{const e=at(t);k(()=>{const i=n=>{if(!(n.ctrlKey&&n.altKey||n.defaultPrevented))switch(n.key){case"Up":case"ArrowUp":n.preventDefault(),e.onUp();break;case"Down":case"ArrowDown":n.preventDefault(),e.onDown();break;case"Enter":n.preventDefault(),e.onEnter();break}};return document.addEventListener("keydown",i,!0),()=>document.removeEventListener("keydown",i,!0)},[e])},uf=({items:t,onSelect:e,defaultIndex:i=0})=>{const[n,s]=ue({index:i}),{index:r}=n,{length:o}=t;return k(()=>{s({index:n.index<0?i:Math.min(n.index,t.length-1),scroll:!0})},[t,i]),df({onUp:V(()=>s(a=>({index:a.index>0?a.index-1:o-1,scroll:!0})),[o]),onDown:V(()=>s(a=>({index:a.index<o-1?a.index+1:0,scroll:!0})),[o]),onEnter:V(()=>r>-1&&r<o&&e?.(t[r],r),[r,t,e])}),{position:n,highlight:V(a=>s({index:a}),[]),select:V(a=>e?.(a),[e])}},pf=(t=it)=>(e,i,{highlight:n,select:s,textual:r=it,query:o,isSelected:a})=>{const l=r(e),c=hf(l,o),h=t(c,e,i);return O`<div
				class="item"
				role="option"
				part="option"
				?aria-selected=${a(e)}
			...
			<div class="sizer" virtualizer-sizer>${h}</div>`},ff=({itemRenderer:t=pf(),...e})=>{const i=at(e);return V((n,s)=>t(n,s,i),[i,t])},mf=t=>{const e=t==="auto",[i,n]=ue(e?40:t);return[i,s=>e?n(s):void 0]},gf=["query","items","onSelect","textual","anchor","itemHeight","itemLimit","itemRenderer","defaultIndex","value","valueProperty","loading"],_f=({value:t,valueProperty:e,items:i,onSelect:n,defaultIndex:s,query:r,textual:o,itemRenderer:a,itemHeight:l=40,itemLimit:c=5})=>{const h=I(()=>cf(t,e),[t,e]),d=I(()=>i.slice(),[i,h]),{position:u,highlight:p,select:f}=uf({items:d,onSelect:n,defaultIndex:isNaN(s)?void 0:Number(s)}),[m,g]=mf(l);return{position:u,items:d,height:Math.min(c,d.length)*m,highlight:p,select:f,itemHeight:m,setItemHeight:g,renderItem:ff({itemRenderer:a,items:d,position:u,highlight:p,select:f,textual:o,query:r,isSelected:h})}},to=ia,vf=t=>{const e=Qo(void 0),{position:i,items:n,renderItem:s,height:r,itemHeight:o,setItemHeight:a}=_f(t);return k(()=>{const l=e.current?.[Qn];l&&l.layoutComplete.then(()=>{t.dispatchEvent(new CustomEvent("layout-complete"));const{averageChildSize:c,averageMarginSize:h}=l._layout._metricsCache;return a(c+h*2)},to)},[n]),k(()=>{if(!i.scroll)return;const l=e.current?.[Qn];if(l){if(!l?._layout){l.layoutComplete.then(()=>l.element(i.index)?.scrollIntoView({block:"nearest"}),to);return}l.element(i.index)?.scrollIntoView({block:"nearest"})}},[i]),tf(af({...i,itemHeight:o,auto:t.itemHeight==="auto"})),O`<div
			class="items"
			style="min-height: ${r}px"
			${Re(l=>e.current=l)}
		>
			<div virtualizer-sizer></div>
			${G1({items:n,renderItem:s,scroller:!0})}
		</div>
		<slot></slot>`};customElements.define("cosmoz-listbox",la(Z(vf,{styleSheets:[Ps(of)]})));const yf=({multi:t,setFloating:e,styles:i,...n},s)=>O`<cosmoz-listbox
		style="${oa(i)}"
...
`;customElements.define("cosmoz-autocomplete-chip",Z(xf,{observedAttributes:["disabled"],styleSheets:[Ps(bf)]}));const io=({content:t,onClear:e,disabled:i,hidden:n,className:s="chip",slot:r})=>O`<cosmoz-autocomplete-chip
		class=${se(s)}
		slot=${se(r)}
		part="chip"
	...
	>`,Cf=({value:t,min:e=0,onDeselect:i,textual:n,disabled:s})=>[...t.filter(Boolean).map(r=>io({content:n(r),onClear:t.length>e&&(()=>i(r)),disabled:s,slot:"control"})),io({content:O`<span></span>`,className:"badge",disabled:!0,slot:"control",hidden:!0})],zf=Ls`
	:host {
		display: block;
		position: relative;
		min-width: 35px;
...
`,Sf=({focused:t,empty:e,...i})=>{const n=t&&e&&i.limit!==1,s=at(i);k(()=>{if(!n)return;const r=o=>{if(o.defaultPrevented)return;const{key:a}=o,l=je(s.value),c=s.limit==1;if(l.length>0&&(a==="Backspace"||c&&a.length===1))return s.onChange(l.slice(0,-1))};return document.addEventListener("keydown",r,!0),()=>document.removeEventListener("keydown",r,!0)},[n,s])},no=t=>t.normalize("NFD").replace(/[\u0300-\u036f]/gu,""),Ef=(t,e,i)=>{if(!e)return t;const n=no(e.toLowerCase()),s=[];for(const r of t){const a=no(i(r).toLowerCase()).indexOf(n);a<0||s.push({item:r,index:a})}return s.sort((r,o)=>r.index-o.index).map(({item:r})=>r)},Mf=t=>t===!1||t==null?[]:t,yl=(t,e,i)=>t.dispatchEvent(new CustomEvent(e,{detail:i})),Af=(t,e,i)=>V(n=>{e?.(n),yl(t,i,n)},[e]),Lf=[],Pf=t=>(...e)=>{let i;const n=()=>{i&&cancelAnimationFrame(i)};return n(),i=requestAnimationFrame(()=>{i=void 0,t(...e)}),n},Hs=(t,e=()=>({}))=>{const i={type:t,toString(){return t}};return Object.assign((...s)=>Object.assign(e(...s),i),i)},so=t=>t.type||t.toString(),ro=t=>Array.isArray(t)?t:[t],Tf=(t,e)=>{const i=ro(e),n=(i.every(Array.isArray)?i:[i]).map(([s,r])=>({actions:ro(s).map(so),handle:r}));return(s=t,r)=>{const o=n.find(a=>a.actions.includes(so(r)));return o?o.handle(s,r):s}},Ve={pending:"pending",rejected:"rejected",resolved:"resolved"},bl={error:void 0,result:void 0,state:Ve.pending},wl=Hs(Ve.pending),xl=Hs(Ve.resolved,t=>({result:t})),Cl=Hs(Ve.rejected,t=>({error:t})),Of=Tf(bl,[[wl,()=>({error:void 0,result:void 0,state:Ve.pending})],[xl,(t,{result:e})=>({error:void 0,result:e,state:Ve.resolved})],[Cl,(t,{error:e})=>({error:e,result:void 0,state:Ve.rejected})]]),kf=t=>{const[{error:e,result:i,state:n},s]=Ch(Of,bl);return k(()=>{if(!t)return;let r=!1;return s(wl()),t.then(o=>!r&&s(xl(o)),o=>!r&&s(Cl(o))),()=>{r=!0}},[t]),[i,e,n]},Hf=({value:t,text:e,onChange:i,onText:n,onSelect:s,limit:r,min:o,source:a,textProperty:l,textual:c,valueProperty:h,keepOpened:d,keepQuery:u,preserveOrder:p,defaultIndex:f,externalSearch:m,...g})=>{const _=I(()=>(c??nf)(l),[c,l]),{active:v,focused:y,onFocus:w,setClosed:x}=sa(g),L=!e,S=I(()=>e?.trim(),[e]),F=qt(),R=Af(F,n,"text"),U=V(E=>{i?.(E,()=>x(!0)),yl(F,"value",E)},[i]),[B,J]=ue([]),K=I(()=>Promise.resolve(typeof a=="function"?a({query:S,active:v}):a).then(Mf),[a,v,S]),H=I(()=>je(t),[t]);k(()=>K.then(J),[K]),Sf({focused:y,empty:L,limit:r,value:H,onChange:U,onText:R}),k(()=>{y||R("")},[y]);const N=at({onText:R,onChange:U,value:H,limit:r,min:o,keepQuery:u,keepOpened:d,setClosed:x,onSelect:s}),[,,D]=kf(K);return{active:v,query:S,textual:_,value:H,source$:K,loading:D==="pending",items:I(()=>{if(!v)return Lf;const E=p?B:[...H,...Cn(H,vl(h))(B)];return m?E:Ef(E,S,_)},[B,v,S,_,L,H,p,h,m]),onClick:V(()=>x(!1),[]),onFocus:w,onText:V(E=>{R(E.target.value),x(!1)},[R,e,x]),onSelect:V(E=>{N.onSelect?.(E,N);const{onChange:T,onText:W,limit:_e,min:Yt,value:Ol,keepQuery:kl,keepOpened:Hl,setClosed:Il}=N;kl||W(""),Hl||Il(!0);const Jt=je(Ol),Is=Jt.includes(E);Is&&Jt.length===Yt||T((Is?Cn(E)(Jt):[...Jt,E]).slice(-_e))},[N]),onDeselect:V(E=>N.onChange(Cn(E)(N.value)),[N]),defaultIndex:S!==void 0&&S?.length>0?0:f}},If=t=>{const e=t.shadowRoot.querySelectorAll(".chip"),i=t.shadowRoot.querySelector(".badge");i.hidden=!0;for(const a of e)a.hidden=!1;const s=t.shadowRoot.querySelector("cosmoz-input").shadowRoot?.querySelector(".control")?.getBoundingClientRect();let r;for(r=0;r<e.length;r++){const l=e[r].getBoundingClientRect();if(!(l.x+l.width<=s.x+s.width-24))break}const o=e.length-r;for(i.querySelector("span").textContent="+"+o.toString(),i.hidden=o<1;r<e.length;r++)e[r].hidden=!0},Nf=({value:t,active:e,wrap:i,limit:n})=>{const s=qt(),r=!(i||n==1),o=I(()=>Pf(()=>If(s)),[]),[a,l]=ue(0);In(()=>{if(!r)return;const c=s.shadowRoot.querySelector("cosmoz-input"),h=new ResizeObserver(d=>{l(d[0].contentRect.width)});return h.observe(c),()=>h.disconnect()},[r]),In(()=>r?o():void 0,[r,a,e,t])},Vf=["input","control","label","line","error","wrap"].map(t=>`${t}: input-${t}`).join(),Rf=[Td({apply({rects:t,elements:e}){Object.assign(e.floating.style,{minWidth:`${Math.max(t.reference.width,t.floating.width)}px`})}}),...ba],Df=({active:t,loading:e,items:i,text:n,isSingle:s,showSingle:r})=>t?(e||i.length>0||n!=null&&n.length>0)&&!(s&&!r):!1,$f=t=>{const{active:e,invalid:i,errorMessage:n,label:s,placeholder:r,disabled:o,noLabelFloat:a,alwaysFloatLabel:l,textual:c,text:h,onText:d,onFocus:u,onClick:p,onDeselect:f,value:m,limit:g,min:_,showSingle:v,items:y,source$:w,placement:x,loading:L}=t,S=qt(),F=g==1,R=F&&m?.[0]!=null,{styles:U,setReference:B,setFloating:J}=wa({placement:x,middleware:Rf});return k(()=>(S.addEventListener("focusin",u),S.addEventListener("focusout",u),()=>{S.removeEventListener("focusin",u),S.removeEventListener("focusout",u)}),[u]),As({focus:()=>S.shadowRoot?.querySelector("#input")?.focus()},[]),O`<cosmoz-input
				id="input"
				part="input"
				${Re(B)}
				.label=${s}
				...
				?readonly=${R}
				?disabled=${o}
				?invalid=${Zn(w.then(()=>i,()=>!0),i)}
				.errorMessage=${Zn(w.then(()=>n,K=>K.message),n)}
				.value=${Ms(h)}
				@value-changed=${d}
				@click=${p}
				autocomplete="off"
			...
			>
				<slot name="prefix" slot="prefix"></slot>
				<slot name="suffix" slot="suffix"></slot>
				${Cf({value:m,min:_,onDeselect:f,textual:c,disabled:o})}
			</cosmoz-input>

			${Ee(Df({active:e,loading:L,items:y,text:h,isSingle:R,showSingle:v}),()=>yf({...t,items:y,multi:!F,setFloating:J,styles:U},Ee(L,()=>O`<cosmoz-autocomplete-skeleton-span></cosmoz-autocomplete-skeleton-span>`,()=>Ee(h!=null&&h.length>0&&y.length===0,()=>O`<slot name="no-result"></slot>`))))}`},zl=t=>{const e={...t,...Hf(t)};return Nf(e),$f(e)},Sl=["disabled","invalid","no-label-float","always-float-label","text-property","value-property","limit","min","show-single","preserve-order","keep-opened","keep-query","default-index","external-search","item-height","item-limit","wrap"],Ff=t=>{const{onChange:e,onText:i,...n}=t;return zl({...n,onChange:V((s,...r)=>{t.value=s,e?.(s,...r)},[e]),onText:V(s=>{t.text=s,i?.(s)},[i])})},El=[Ps(zf)];customElements.define("cosmoz-autocomplete-ui",Z(zl,{observedAttributes:Sl,styleSheets:El}));customElements.define("cosmoz-autocomplete",Z(Ff,{observedAttributes:Sl,styleSheets:El}));const Bf=xe`
	@keyframes rotating {
		100% {
