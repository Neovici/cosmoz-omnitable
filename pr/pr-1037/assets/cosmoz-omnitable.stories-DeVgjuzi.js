...
`})),Vi,Hi,Ui=e((()=>{Se(),Vi=e=>{let t=e.replace(/"/gu,`""`);return t.search(/("|,|\n)/gu)>=0?`"`+t+`"`:e},Hi=(e,t,n)=>{if(t.length===0)return;let r=e.map(e=>Vi(e.title)).join(`;`)+`
`,i=t.map(t=>e.map(e=>{let n=e.getString(e,t);return n==null?``:Vi(String(n))}).join(`;`)+`
`);i.unshift(r),Ke(new File(i,n,{type:`text/csv;charset=utf-8`}))}})),Wi,Gi,Ki=e((()=>{We(),Se(),Wi=(e,t)=>{let n=e.map(e=>e.title),r=t.map(t=>e.map(e=>e.toXlsxValue(e,t)??``));return r.unshift(n),r},Gi=(e,t,n,r)=>{if(t.length===0)return;let i=Wi(e,t),a=new Qe(n).addSheetFromData(i,r).generate();Ke(new File([a],n,{type:`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`}))}})),qi,W,Ji,Yi=e((()=>{qi=Symbol(`index`),W=Symbol(`All`),Ji=(e,t)=>{if(typeof e.findLastIndex==`function`)return e.findLastIndex(t);for(let n=e.length-1;n>=0;n--)if(t(e[n],n,e))return n;return-1}})),Xi,Zi=e((()=>{s(),i(),o(),Ui(),Ki(),Yi(),Xi=({columns:e,selectedItems:t,setSelectedItems:n,csvFilename:i,xlsxFilename:o,xlsxSheetname:s,topPlacement:c,enableSelectAll:l,matchingItemsCount:u=0})=>{let d=t===W?u:t.length;return a`<cosmoz-bottom-bar
		id="bottomBar"
		?active=${!Ae(d)}
		part="bottomBar"
	...
	>
		<span slot="info">
			${t===W?r(`All {count} matching items selected`,{count:u}):r(`{count} selected item`,{count:d})}
		</span>
		${t!==W&&l&&d>0?a`<button @click=${()=>n(W)}>
