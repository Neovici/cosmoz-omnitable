...
`,co=e=>e?`groupRow groupRow-folded`:`groupRow`,lo=({item:e,index:t})=>n=>A(n?.length>0,()=>a`
	...
	`},customElements.define(`cosmoz-omnitable`,m(es,{observedAttributes:[`hash-param`,`sort-on`,`group-on`,`descending`,`group-on-descending`,`hide-select-all`,`settings-id`,`no-local`,`no-local-sort`,`no-local-filter`,`disabled-filtering`,`loading`,`mini-breakpoint`,`inline`,`enable-select-all`]})),ts=`
