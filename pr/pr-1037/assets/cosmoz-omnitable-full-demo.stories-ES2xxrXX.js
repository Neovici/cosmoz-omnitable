import{e as d,w as Jt,D as Le,t as C,A as Re}from"./iframe-aYJXMqzL.js";import{A as xe,n as F,c as A,t as q,g as k,P as T,s as Yt,a as zt,r as Zt,e as en,D as tn,b as nn,d as on,i as we,h as St,m as rn,u as N,f as Ct,j as kt,k as ln,l as Y,p as B,o as Me,q as _t,v as ze,w as an,x as Se,y as O,z as E,B as Ce,C as It,E as nt,F as ot,G as S,H as sn,I as un,J as Ft,K as Be,L as Ot,M as cn,N as Dt,O as dn,Q as He,R as rt,S as mn,T as pn,U as gn,V as Ne,W as Ge,X as hn,Y as Vt,Z as je,_ as U,$ as fn,a0 as bn}from"./cosmoz-collapse-BT7vJ0JR.js";import"./preload-helper-PPVm8Dsz.js";const xn=({slot:n,title:e,className:t,width:o="24",height:r="24",styles:i}={})=>d`
  <svg
    slot=${xe(n)}
    class=${`delete-icon ${t??""}`}
    viewBox="0 0 24 24"
...
`;customElements.define("cosmoz-omnitable-settings",A($r,{styleSheets:[Se(fr)]}));const ye=["sortOn","descending","groupOn","groupOnDescending"],me=n=>e=>typeof e=="object"&&e!==null&&"name"in e&&e.name===n,wr=(n=[],e=[],t=[])=>{const o=e.filter(l=>n.some(me(l.name))),r=n.filter(l=>l.name!=null&&!e.some(me(l.name))&&!t.some(me(l.name))),i=t.filter(l=>!e.some(me(l.name)));return[...o,...i.flatMap(l=>{const a=n.find(s=>s.name===l.name);return a?{...l,title:a.title??l.title??"",minWidth:parseInt(a.minWidth??"0",10)}:[]}),...r.map(l=>{const{name:a,title:s,priority:u,minWidth:m,width:c,flex:g}=l;return{name:a??"",title:s??"",priority:u??0,minWidth:parseInt(m??"0",10),width:parseInt(c??"0",10),flex:parseInt(g??"0",10)}})]},zr=(n,e)=>({...e,...Ne(Array.from(ye))(n),columns:n.columns?.map(Ne(["name","priority","width","flex","disabled"]))}),Sr=({columns:n,settings:e,savedSettings:t,initial:o})=>({...Object.fromEntries(ye.flatMap(i=>o?.[i]!=null?[[i,o[i]]]:[])),...t?Ne(Array.from(ye))(t):{},...e,columns:wr(n,e?.columns,t?.columns)}),Cr=({prefix:n="omnitable-"}={})=>({write:async(t,o)=>{const r=n+t;try{o?localStorage.setItem(r,JSON.stringify(o)):localStorage.removeItem(r)}catch(i){console.error(i)}},read:async t=>{if(!t)return null;try{const o=localStorage.getItem(n+t);return o==null?null:JSON.parse(o)}catch(o){return console.error(o),null}}}),kr=Ft(Cr),_r=()=>{const n=Be(kr);return O(()=>n(),[n])},Ir=(n,e,t,o)=>{const[r,i]=N(n?void 0:null),{read:l,write:a}=_r();return E(async()=>{n&&i(await l(n))},[n,l]),{settingsId:n,savedSettings:r,onSave:S(async()=>{if(!n)return;const s=zr(e,r);await a(n,s),t(),i(s)},[e,r]),onReset:S(async s=>{t(),s.shiftKey&&(await a(n),i(null)),o?.()},[o]),hasChanges:e!=null}},Fr=({settingsId:n,host:e})=>{const t=O(()=>Object.fromEntries(ye.map(x=>[x,e[x]])),[]),o=Ge(),r=S(()=>{o.current?.(t)},[t]),[i,l]=N(),[a,s]=N({columns:!0,sort:!0}),{savedSettings:u,...m}=Ir(n,i,l,r),{enabledColumns:c,disabledFiltering:g}=e,p=Qn(e,{enabledColumns:c,disabledFiltering:g}),h=O(()=>Sr({columns:p,settings:i,savedSettings:u,initial:t}),[p,i,u]),$=O(()=>h.columns.map(x=>p.find(b=>b.name===x.name)).filter(Boolean),[p,...h.columns.map(x=>x.name)]);return{...m,savedSettings:u,opened:a,setOpened:s,settings:h,columns:$,setSettings:l,resetRef:o}},yt=n=>Number.isFinite(n)?n:0,Or=(n,e)=>{const t=[];let[o,r]=n.reduce(([m,c],{width:g,flex:p})=>[m+g,c+p],[0,0]),i=e-o,l=yt(i/r),a=0,s=0,u=0;for(let m=0;m<n.length;m++){const{width:c,minWidth:g,flex:p}=n[m],h=i>=0?l*p:c*i/o;if(g>c+h){a+=c,s+=g,u+=p,t[m]=g;continue}if(p===0){a+=c,s+=c,t[m]=c;continue}}o-=a,i=e-s-o,r-=u,l=yt(i/r);for(let m=0;m<n.length;m++){if(t[m]!=null)continue;const{width:c,flex:g}=n[m],p=i>=0?l*g:c*i/o;t[m]=c+p}return t},Dr=(n,e)=>{const t=ur(n,l=>l!=null&&l>0),o=(l,a)=>`.cell[name="${l}"], cosmoz-omnitable-skeleton::part(cell-${l}){width: ${Math.floor(a)}px;padding: 0 min(3px, ${a/2}px)}`,r=l=>`cosmoz-omnitable-resize-nub[name="${l}"]{display:none}`,i=l=>`cosmoz-omnitable-resize-nub[name="${l}"], .cell[name="${l}"]{display:none}`;return e.map((l,a)=>{const s=n[a];if(s==null||s===0)return i(l.name);const u=o(l.name,s);return a===t?`${u}
${r(l.name)}`:u}).join(`
