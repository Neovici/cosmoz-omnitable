import {
	component, useRef, useCallback, useEffect, useMemo
} from 'haunted';
...
const
	...
	LayoutHelper = function ({ config }) {
		const container = useRef(),
			dispatchEvent = useCallback(event => this.dispatchEvent(event), []),
			dropConfig = useMemo(
				() =>
					Object.fromEntries(
			...
			{
				reset, dimensions, hiddenColumns
			} = useColumnAdjust(
				container,
				dropConfig
			);
		...
		return html`
			...
			<div class="row" ref="${ref(container)}">${layoutRow(
				config,
				hiddenColumns
			)}</div>
		`;
